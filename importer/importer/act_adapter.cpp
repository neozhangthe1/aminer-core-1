...
{
	//向_ACTa2countMap[tid]中<actxid, count++>
	...
	//向_ACTa2countMap[tid]中<-1, count++>
	sumMap[tid] ++;

	//向_ACTa2countMap[ntopic]中<actxid, count++>
	...
	//向_ACTa2countMap[ntopic]中<-1, count++>
	sumMap[nTopic] ++;
}
...
int ACT::getAuthorNaIdByACTaid(int actaid)//DONE
{
	...
	set<int>* authors = name2Author.getAuthorsByAuthorName(name);
	if (authors && authors->size() > 0)
	{
		Author* author = (Author*)resource.authorCollection()->getDocumentByIndex(*authors->begin());
		if(author)
			return author->naid;
	}
	return -1;
...
int ACT::getConfIdByACTcid(int actcid)//DONE
{
	...
	int confid = jConfMapInstance->getIdByName(confname);
	return confid;
}
...
int ACT::getACTaidByAuthorNaId(int naid)//DONE
{
	if(act_aid_map.empty())
	{
		...
		if(act_aid_map.empty())
		{
			act_aid_map.resize(IndexResource::instance().authorCollection()->getDocCounts(), ACT_CACHE_UNINIT);
		}
	...
	Author* author = (Author*)acollection->getDocumentByIndex(naid);
	if(author==NULL)
		return -1;
...
int ACT::getACTcidByConfid(int confid)//DONE
{
	if(!act_conf_map.size())
	{
		...
		{
			//in case of error
			act_conf_map.resize(jConfMapInstance->getSize()+1000, ACT_CACHE_UNINIT);
		}
	}
	...
	string confname = jConfMapInstance->getName(confid);
	unordered_map<string, int>::iterator iter = _reauthormap.find(confname);
	int actcid = -1;
...
void ACT::_init()//DONE
{
	...
	jConfMapInstance = JConfMap::getInstance();
}
...
vector<double> ACTadapter::getTopicDistributionGivenPub(int pubid, Publication const * pub)//DONE
{
	...
	{
        if (!pub)
            pub = (Publication*)IndexResource::instance().pubCollection()->getDocumentByIndex(pubid);
		if (pub)
...
{
	int confid = _act.jConfMapInstance->getIdByName(confname);
...
bool ACTadapter::getTopTopicGivenPub(vector<pair<int, double>>& result, int pub_id)
{
	...
	if(pid==-1)
	{
		Publication* pub = (Publication*)IndexResource::instance().pubCollection()->getDocumentByIndex(pub_id);
		if(!pub)
			return false;
